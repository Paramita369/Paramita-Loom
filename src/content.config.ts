...
const guideEvidenceRefSchema = z
  .object({
    label: z.string().min(1).optional(),
...
const loomDocsSchema = z.object({
  // === Required ===
  title: z.string(),

  // === From Starlight docsSchema (preserved for content compatibility) ===
  description: z.string().optional(),
  editUrl: z.union([z.string(), z.boolean()]).optional().default(true),
  head: z
    .array(
      z.object({
        tag: z.string(),
  ...
  tableOfContents: z
    .union([
      z.object({
        minHeadingLevel: z.number().default(2),
        maxHeadingLevel: z.number().default(3),
      }),
      z.boolean(),
    ])
    .optional(),
  ...
  prev: z
    .union([
      z.string(),
      z.object({ label: z.string().optional(), link: z.string().optional() }),
      z.boolean(),
    ])
    .optional(),
  next: z
  ...
  // === Custom fields (unchanged from previous content.config.ts) ===
